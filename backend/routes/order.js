...
router.post('/create', (req, res) => {
    ...
    console.log("Thông tin đơn hàng nhận được từ frontend:", req.body);

    db.beginTransaction((err) => {
        if (err) return res.status(500).json({ message: 'Lỗi khi bắt đầu transaction' });
        ...
        db.query(orderQuery, [address, phone_number, total, acc_id, full_name], (err, orderResult) => {
            ...
            const itemPromises = orderItems.map((item) => {
                const { product_id, quantity_items, price } = item;  // Lấy quantity_items từ frontend
                    ...
                    db.query(orderItemQuery, [orderId, product_id, quantity_items, price], (err) => {
                        ...
                        db.query(updateProductQuery, [quantity_items, product_id, quantity_items], (err, result) => {
                            ...
                            if (result.affectedRows === 0) {
                                console.error(`Sản phẩm ${product_id} không đủ số lượng hoặc có lỗi khi cập nhật`);
